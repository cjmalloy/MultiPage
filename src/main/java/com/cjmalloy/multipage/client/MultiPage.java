...
{
    private static Map<String, PageFactory> pages = new HashMap<String, PageFactory>();
    ...
    {
        if (transition != null) p.setStyleName(transition.initial(), true);
        RootPanel.get().add(p, 0, 0);
        if (currentPage != null)
        {
            if (transition != null) currentPage.setStyleName(transition.out(), true);
            currentPage.remove(removeDelay);
            currentPage.onHide();
        }
        p.onShow();
        currentPage = p;
        if (transition != null) Scheduler.get().scheduleDeferred(new ScheduledCommand()
        {
            @Override
            public void execute()
            {
    ...
    public void setPage(final String historyToken)
    {
        ...
        if (pages.containsKey(historyToken)) f = pages.get(historyToken);

        if (f != null) f.create(new Callback<Page, Throwable>()
        {
            @Override
